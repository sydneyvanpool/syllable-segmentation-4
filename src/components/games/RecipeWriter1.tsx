...
import { Volume2, ArrowRight, BookOpen, CheckCircle2, VolumeX } from 'lucide-react';
import { SpeakableText } from '../SpeakableText';
import { RECIPE_SENTENCES_LEVEL_1 } from '../../data/syllableData';
import { QuestionResult } from '../../types';
import { playSound, speakText, stopSpeaking } from '../../utils/audio';
import { firePastryConfetti } from '../../utils/confetti';
import { SummaryModal } from '../SummaryModal';

interface RecipeWriter1Props {
  onComplete: (results: QuestionResult[]) => void;
  onHome: () => void;
  onNextGame: () => void;
}

export const RecipeWriter1: React.FC<RecipeWriter1Props> = ({
  onComplete,
  onHome,
  onNextGame,
}) => {
  const targetCorrect = RECIPE_SENTENCES_LEVEL_1.length;
  const [questions, setQuestions] = useState(RECIPE_SENTENCES_LEVEL_1);
  ...
  const handleSelectWord = (word: string, syllables: string[], count: number) => {
    ...
    if (correct) {
      ...
      const randomTask = RECIPE_SENTENCES_LEVEL_1[Math.floor(Math.random() * RECIPE_SENTENCES_LEVEL_1.length)];
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-4 sm:p-6 space-y-6">
      ...
      <div className="bg-amber-100/90 border-2 border-amber-300 rounded-3xl p-4 sm:p-5 shadow-sm text-center relative overflow-hidden">
        ...
        <div className="flex flex-col items-center gap-1">
          ...
          <p className="text-sm sm:text-base text-amber-900 font-medium">
            <SpeakableText
              text="Read the bakery recipe. Pick the word with the correct number of syllables to finish the recipe!"
              buttonSize="sm"
            />
          </p>
        </div>
      ...
      {/* Recipe Notepad Visual */}
      <div className="bg-[#fffdf5] rounded-3xl border-3 border-amber-300 p-6 sm:p-8 shadow-lg relative space-y-6">
        {/* Notebook top binding decoration */}
        <div className="flex justify-center gap-4 -mt-2 mb-2">
          {[...Array(6)].map((_, i) => (
            <div key={i} className="w-3 h-3 rounded-full bg-amber-200 border border-amber-400" />
          ))}
        </div>

        {/* Decodable Sentence display */}
            ...
            <span
              className={`inline-block px-2.5 py-0.5 mx-1 rounded-xl border-2 font-black transition-all align-middle ${
                selectedWord
                  ...
                  : 'bg-white border-amber-300 text-stone-400 min-w-[100px] text-center'
              }`}
            >
              {selectedWord || '______'}
            </span>{' '}
        ...
        >
          <div className="space-y-1 text-center sm:text-left">
            <div className="font-extrabold text-lg">
              {isCorrect ? '🎉 Delicious! Recipe step completed!' : '💡 Check the syllables:'}
            </div>
